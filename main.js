      ...
      models: [
        {
          ...
          name: "Claude 4 Opus - Top Performance",
          maxTokens: 2e5,
          supportedLanguages: ["korean", "english"],
        ...
        },
        {
          ...
          name: "Claude 3.5 Haiku - Fast Response",
          maxTokens: 2e5,
          supportedLanguages: ["korean", "english"],
      ...
      models: [
        {
          ...
          name: "Gemini 2.5 Pro Preview - Top Performance",
          maxTokens: 1048576,
          supportedLanguages: ["korean", "english"],
        ...
        {
          id: "gemini-2.5-flash",
          name: "Gemini 2.5 Flash - Stable Version",
          maxTokens: 1048576,
          supportedLanguages: ["korean", "english"],
...
${prompt}`
    ...
    try {
      ...
      const data = response.json;
      if (request.settings.showDebugInfo) {
        console.log("=== Gemini API Detailed Response Analysis ===");
        console.log("Full response data:", data);
        console.log("candidates exists:", !!data.candidates);
        console.log("candidates length:", ((_a = data.candidates) == null ? void 0 : _a.length) || 0);
        if (data.candidates && data.candidates.length > 0) {
          const candidate2 = data.candidates[0];
          console.log("First candidate:", candidate2);
          console.log("candidate.content exists:", !!candidate2.content);
          console.log("candidate.content:", candidate2.content);
          if (candidate2.content) {
            console.log("content.parts exists:", !!candidate2.content.parts);
            console.log("content.parts length:", ((_b = candidate2.content.parts) == null ? void 0 : _b.length) || 0);
            console.log("content.parts:", candidate2.content.parts);
            if (candidate2.content.parts && candidate2.content.parts.length > 0) {
              console.log("First part:", candidate2.content.parts[0]);
              console.log("First part.text:", candidate2.content.parts[0].text);
            }
          }
          console.log("candidate.text exists:", !!candidate2.text);
          console.log("candidate.output exists:", !!candidate2.output);
          console.log("All candidate keys:", Object.keys(candidate2));
        }
      ...
      if (!data.candidates || data.candidates.length === 0) {
        if (request.settings.showDebugInfo) {
          console.error("\u274C No candidates or empty array");
      ...
      if (request.settings.showDebugInfo) {
        ...
        if (candidate.finishReason === "MAX_TOKENS") {
          console.warn("\u26A0\uFE0F Response may be truncated due to MAX_TOKENS");
        }
      }
      if (candidate.content && candidate.content.parts && candidate.content.parts.length > 0) {
        ...
        if (part.text !== void 0) {
          ...
          if (request.settings.showDebugInfo) {
            console.log("\u2705 Text extracted from content.parts[0].text:", extractedText.length > 0 ? "Success" : "Empty text");
          }
        }
      } else if (candidate.text !== void 0) {
        extractedText = candidate.text || "";
        if (request.settings.showDebugInfo) {
          console.log("\u2705 Text extracted from candidate.text:", extractedText.length > 0 ? "Success" : "Empty text");
        }
      } else if (candidate.output !== void 0) {
        extractedText = candidate.output || "";
        if (request.settings.showDebugInfo) {
          console.log("\u2705 Text extracted from candidate.output:", extractedText.length > 0 ? "Success" : "Empty text");
        }
      }
      if (candidate.finishReason === "MAX_TOKENS") {
        if (request.settings.showDebugInfo) {
          console.log("\u26A0\uFE0F MAX_TOKENS detected - attempting to use partial result");
          console.log("\u{1F9E0} thinking tokens info:", ((_d = data.usageMetadata) == null ? void 0 : _d.thoughtsTokenCount) || "N/A");
        }
        if (!extractedText || extractedText.trim().length === 0) {
          const thoughtsCount = ((_e = data.usageMetadata) == null ? void 0 : _e.thoughtsTokenCount) || 0;
          if (thoughtsCount > 0) {
            extractedText = `[Gemini 2.5 model reached token limit using ${thoughtsCount} thinking tokens. For longer responses, please increase maxCompletionLength in settings.]`;
          } else {
            extractedText = "[Response truncated due to token limit]";
          }
        }
      } else if (!extractedText || extractedText.trim().length === 0) {
        if (request.settings.showDebugInfo) {
          console.error("\u274C Failed to extract text using all methods");
...
var AIWritingAssistantSettingsTab = class extends import_obsidian2.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h1", { text: "AI Writing Assistant Settings" });
    new import_obsidian2.Setting(containerEl).setName("Enable Plugin").setDesc("Toggle AI completion feature on/off").addToggle((toggle) => toggle.setValue(this.plugin.settings.enabled).onChange(async (value) => {
    ...
    new import_obsidian2.Setting(containerEl).setName("AI Provider").setDesc("Select the AI service to use").addDropdown((dropdown) => dropdown.addOptions(providerOptions).setValue(this.plugin.settings.selectedProvider).onChange(async (value) => {
    ...
    if (selectedProvider) {
      ...
      new import_obsidian2.Setting(containerEl).setName("AI Model").setDesc("Select the AI model to use").addDropdown((dropdown) => dropdown.addOptions(modelOptions).setValue(this.plugin.settings.selectedModel).onChange(async (value) => {
        this.plugin.settings.selectedModel = value;
        await this.plugin.saveSettings();
      }));
      if (selectedProvider.requiresApiKey) {
        new import_obsidian2.Setting(containerEl).setName(`${selectedProvider.name} API Key`).setDesc(`API key is required to use ${selectedProvider.name} service`).addText((text) => text.setPlaceholder("Enter your API key").setValue(this.plugin.settings.apiKeys[selectedProvider.id] || "").onChange(async (value) => {
          this.plugin.settings.apiKeys[selectedProvider.id] = value;
          await this.plugin.saveSettings();
        }));
      }
    }
    containerEl.createEl("h2", { text: "Writing Settings" });
    new import_obsidian2.Setting(containerEl).setName("Language").setDesc("Select your primary language").addDropdown((dropdown) => dropdown.addOptions({
      "korean": "Korean",
      "english": "English",
      "auto": "Auto Detect"
    ...
    new import_obsidian2.Setting(containerEl).setName("Writing Style").setDesc("Select your preferred writing style").addDropdown((dropdown) => dropdown.addOptions(styleOptions).setValue(this.plugin.settings.writingStyle).onChange(async (value) => {
    ...
    if (this.plugin.settings.writingStyle === "custom") {
      if (this.plugin.settings.customWritingStyles.length > 0) {
        ...
        new import_obsidian2.Setting(containerEl).setName("Select Custom Style").setDesc("Select a custom writing style to use").addDropdown((dropdown) => dropdown.addOptions(customStyleOptions).setValue(this.plugin.settings.selectedCustomStyle || "").onChange(async (value) => {
          this.plugin.settings.selectedCustomStyle = value;
          await this.plugin.saveSettings();
        }));
      }
      new import_obsidian2.Setting(containerEl).setName("Add New Custom Style").setDesc("Create your own writing style").addButton((button) => button.setButtonText("Add New Style").setCta().onClick(() => {
      ...
      this.plugin.settings.customWritingStyles.forEach((style, index) => {
        new import_obsidian2.Setting(containerEl).setName(style.name).setDesc(`${style.description} (Formality: ${style.formality}, Creativity: ${Math.round(style.creativity * 100)}%)`).addButton((button) => button.setButtonText("Edit").onClick(() => {
    ...
    new import_obsidian2.Setting(containerEl).setName("Completion Type").setDesc("Select the length of AI completions").addDropdown((dropdown) => dropdown.addOptions({
      "sentence": "Sentence",
      "paragraph": "Paragraph",
      "bullet": "Bullet List",
      "auto": "Auto"
    ...
    new import_obsidian2.Setting(containerEl).setName("Completion Delay").setDesc("Time to wait after stopping typing before triggering AI completion (milliseconds)").addSlider((slider) => slider.setLimits(500, 5e3, 250).setValue(this.plugin.settings.delayMs).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.delayMs = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Max Completion Length").setDesc("Maximum number of words AI will generate").addSlider((slider) => slider.setLimits(20, 500, 20).setValue(this.plugin.settings.maxCompletionLength).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.maxCompletionLength = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Creativity Level").setDesc("AI response creativity (0.0 = conservative, 1.0 = creative)").addSlider((slider) => slider.setLimits(0.1, 1, 0.1).setValue(this.plugin.settings.temperature).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.temperature = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Minimum Input Characters").setDesc("Minimum number of characters required before triggering AI completion").addSlider((slider) => slider.setLimits(5, 50, 5).setValue(this.plugin.settings.minCharsBeforeCompletion).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.minCharsBeforeCompletion = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h2", { text: "Advanced Settings" });
    new import_obsidian2.Setting(containerEl).setName("Auto-detect Context").setDesc("Analyze surrounding context to improve completions").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoDetectContext).onChange(async (value) => {
      this.plugin.settings.autoDetectContext = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Enable Cache").setDesc("Process repeated completion requests faster").addToggle((toggle) => toggle.setValue(this.plugin.settings.cacheEnabled).onChange(async (value) => {
      this.plugin.settings.cacheEnabled = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian2.Setting(containerEl).setName("Show Debug Info").setDesc("Output debug information to console").addToggle((toggle) => toggle.setValue(this.plugin.settings.showDebugInfo).onChange(async (value) => {
      this.plugin.settings.showDebugInfo = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h2", { text: "\u{1F4CA} Usage Statistics" });
  ...
  addUsageStatsSection(containerEl) {
    ...
    totalSection.createEl("h3", { text: "\u{1F522} Total Usage" });
    new import_obsidian2.Setting(totalSection).setName("Total Completions").setDesc(`${stats.totalCompletions.toLocaleString()} times`).setClass("usage-stat-item");
    new import_obsidian2.Setting(totalSection).setName("Total Tokens Used").setDesc(`${stats.totalTokensUsed.toLocaleString()} tokens`).setClass("usage-stat-item");
    new import_obsidian2.Setting(totalSection).setName("Total Estimated Cost").setDesc(`$${stats.totalCost.toFixed(4)} USD`).setClass("usage-stat-item");
    ...
    warningDiv.innerHTML = `
			<div style="margin: 10px 0; padding: 10px; background-color: var(--background-modifier-warning); border-radius: 5px; border-left: 4px solid var(--color-orange);">
				...
				\u2022 Token costs shown are <strong>approximate estimates</strong><br>
				\u2022 Actual charges may vary based on each API provider's exact pricing<br>
				\u2022 Please check each provider's official dashboard for accurate costs
			</div>
		`;
    new import_obsidian2.Setting(containerEl).setName("Detailed Usage Info").setDesc(this.plugin.settings.showDetailedStats ? "Click to hide detailed information" : "Click to view usage by provider, daily, and monthly").addButton((button) => button.setButtonText(this.plugin.settings.showDetailedStats ? "\u25B2 Hide Details" : "\u25BC Show Details").onClick(async () => {
    ...
    if (this.plugin.settings.showDetailedStats) {
      ...
      new import_obsidian2.Setting(detailsContainer).setName("Average Response Time").setDesc(`${Math.round(stats.averageLatency)}ms`).setClass("usage-stat-item");
      if (Object.keys(stats.providerUsage).length > 0) {
        const providerSection = detailsContainer.createDiv("usage-stats-section");
        providerSection.createEl("h3", { text: "\u{1F3E2} Usage by Provider" });
        for (const [providerId, tokens] of Object.entries(stats.providerUsage)) {
          ...
          new import_obsidian2.Setting(providerSection).setName(providerName).setDesc(`${tokens.toLocaleString()} tokens`).setClass("usage-stat-item");
        }
      }
      const recentDays = this.getRecentDays(7);
      if (recentDays.length > 0) {
        const recentSection = detailsContainer.createDiv("usage-stats-section");
        recentSection.createEl("h3", { text: "\u{1F4C5} Recent 7 Days Usage" });
        for (const day of recentDays) {
          const dayStats = stats.dailyUsage[day.key];
          if (dayStats && (dayStats.tokens > 0 || dayStats.cost > 0)) {
            new import_obsidian2.Setting(recentSection).setName(day.label).setDesc(`${dayStats.tokens.toLocaleString()} tokens \u2022 $${dayStats.cost.toFixed(4)}`).setClass("usage-stat-item");
          }
        }
      }
      ...
      if (recentMonths.length > 0) {
        ...
        monthlySection.createEl("h3", { text: "\u{1F4CA} Monthly Usage" });
        for (const month of recentMonths) {
          const monthStats = stats.monthlyUsage[month.key];
          if (monthStats && (monthStats.tokens > 0 || monthStats.cost > 0)) {
            new import_obsidian2.Setting(monthlySection).setName(month.label).setDesc(`${monthStats.tokens.toLocaleString()} tokens \u2022 $${monthStats.cost.toFixed(4)}`).setClass("usage-stat-item");
          }
        }
      }
    }
    new import_obsidian2.Setting(containerEl).setName("Reset Statistics").setDesc("Reset all usage statistics (cannot be undone)").addButton((button) => button.setButtonText("Reset Statistics").setWarning().onClick(async () => {
      ...
      await this.plugin.saveSettings();
      new import_obsidian2.Notice("Usage statistics have been reset.");
      this.display();
    }));
  }
  getRecentDays(count) {
...
var CustomStyleModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    contentEl.empty();
    contentEl.createEl("h2", { text: this.editingStyle ? "Edit Custom Style" : "Create New Custom Style" });
    new import_obsidian2.Setting(contentEl).setName("Style Name").setDesc("Enter a name for this style").addText((text) => {
      var _a2;
      this.nameInput = text;
      text.setPlaceholder("e.g., Academic Writing Style").setValue(((_a2 = this.editingStyle) == null ? void 0 : _a2.name) || "");
    });
    new import_obsidian2.Setting(contentEl).setName("Style Description").setDesc("Enter a brief description of this style").addText((text) => {
      var _a2;
      this.descInput = text;
      text.setPlaceholder("e.g., Academic and precise expressions").setValue(((_a2 = this.editingStyle) == null ? void 0 : _a2.description) || "");
    });
    new import_obsidian2.Setting(contentEl).setName("System Prompt").setDesc("Enter instructions for AI to follow when writing in this style").addTextArea((text) => {
      var _a2;
      this.promptInput = text;
      text.setPlaceholder("e.g., You write in academic and precise language. Use appropriate technical terms, maintain logical structure...").setValue(((_a2 = this.editingStyle) == null ? void 0 : _a2.systemPrompt) || "").inputEl.rows = 5;
    });
    new import_obsidian2.Setting(contentEl).setName("Tone & Mood").setDesc("Describe the tone and mood of the writing").addText((text) => {
      var _a2;
      this.toneInput = text;
      text.setPlaceholder("e.g., Calm and careful").setValue(((_a2 = this.editingStyle) == null ? void 0 : _a2.tone) || "");
    });
    new import_obsidian2.Setting(contentEl).setName("Formality").setDesc("Select the formality level of the writing").addDropdown((dropdown) => {
      var _a2;
    ...
    new import_obsidian2.Setting(contentEl).setName("Creativity Level").setDesc("0 = more accurate and consistent expressions, 1 = more creative and varied expressions").addSlider((slider) => {
      var _a2;
    ...
    exampleContainer.createEl("h3", { text: "Example Sentences (Optional)" });
    exampleContainer.createEl("p", {
      text: "Adding example sentences written in this style helps AI learn the style more accurately.",
      cls: "setting-item-description"
    });
    ...
    const renderExamples = () => {
      ...
      examples.forEach((example, index) => {
        const exampleSetting = new import_obsidian2.Setting(examplesList).setName(`Example ${index + 1}`).addTextArea((text) => {
          text.setValue(example).setPlaceholder("Enter an example sentence written in this style");
          text.inputEl.rows = 2;
      ...
      new import_obsidian2.Setting(examplesList).addButton((button) => button.setButtonText("Add Example").onClick(() => {
        examples.push("");
        renderExamples();
      }));
    };
...
var AIWritingAssistantPlugin = class extends import_obsidian3.Plugin {
  ...
  async onload() {
    console.log("\u{1F680} AI Writing Assistant Plugin loading started");
    await this.loadSettings();
    console.log("\u2699\uFE0F Settings loaded:", this.settings.enabled);
    this.aiService = new AIService();
    this.editorHandler = new EditorHandler();
    this.ghostTextManager = new GhostTextManager();
    console.log("\u{1F4DD} Registering Ghost Text Extension...");
    this.registerEditorExtension(ghostTextExtension);
    console.log("\u2705 Ghost Text Extension registered");
  ...
  setupEditorHandlers() {
    console.log("\u{1F3AF} Setting up editor handlers");
    ...
    console.log("\u{1F4E1} Custom event listener registered: aiAutoCompletionTrigger");
    this.registerDomEvent(document, "keydown", this.onKeyDown.bind(this));
    console.log("\u2328\uFE0F Keyboard event listener registered");
    console.log("\u2705 Editor handlers setup complete (CodeMirror 6 mode)");
  }
  async onAutoCompletionTrigger(event) {
    const customEvent = event;
    console.log("Auto-completion event received:", customEvent.detail);
    if (!this.settings.enabled || this.isProcessingCompletion) {
      console.log("Auto-completion disabled or processing");
      return;
    }
  ...
  async triggerCompletion(editor, view) {
    ...
    try {
      ...
      this.updateStatusBar("AI completion generating...");
      ...
      this.updateStatusBar(`AI completion ready (${completion.model})`);
      ...
      console.error("AI completion error:", error);
      this.ghostTextManager.clearGhostText();
      this.updateStatusBar("AI completion failed");
      let errorMessage = "Cannot generate AI completion.";
      if (error.message.includes("API key")) {
        errorMessage = "Please check your API key.";
      } else if (error.message.includes("network") || error.message.includes("fetch")) {
        errorMessage = "Please check your network connection.";
      } else if (error.message.includes("Ollama")) {
        errorMessage = "Please check if Ollama server is running.";
      }
      if (this.settings.showDebugInfo) {
...
Debug: ${error.message}`);
  ...
  addCommands() {
    this.addCommand({
      ...
      name: "Toggle AI Completion",
      callback: () => {
        this.settings.enabled = !this.settings.enabled;
        this.saveSettings();
        if (this.settings.enabled) {
          this.enableCompletion();
          new import_obsidian3.Notice("AI completion enabled.");
        } else {
          this.disableCompletion();
          new import_obsidian3.Notice("AI completion disabled.");
        }
      }
    });
    this.addCommand({
      id: "trigger-ai-completion",
      name: "Trigger AI Completion Manually",
      editorCallback: async (editor, ctx) => {
        if (!this.settings.enabled) {
          new import_obsidian3.Notice("AI completion is disabled.");
          return;
        }
    ...
    this.addCommand({
      ...
      callback: () => {
        ...
        new import_obsidian3.Notice("AI cache cleared.");
      }
    });
  }
  setupStatusBar() {
    this.statusBarItem = this.addStatusBarItem();
    this.updateStatusBar("AI completion ready");
  }
  ...
  enableCompletion() {
    ...
    this.updateStatusBar("AI completion enabled");
  ...
  disableCompletion() {
    ...
    this.updateStatusBar("AI completion disabled");
  ...
  async testCompletion() {
    ...
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian3.MarkdownView);
    if (!activeView) {
      new import_obsidian3.Notice("No markdown view.");
      return;
    }
    new import_obsidian3.Notice("Starting AI completion test...");
    await this.triggerCompletion(editor, activeView);
  }
  getDebugInfo() {
    return {
